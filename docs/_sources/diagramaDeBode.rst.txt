...
.. container:: toggle, toggle-hidden
	...
	.. exec_code:: realCTsignals signalplots
		...
		plt.ylabel("Fase (deg)")
		plt.xlabel("Frequência (rad/s)")
		plt.savefig('source/figures/BodeProva1.png')
...
.. container:: toggle, toggle-hidden
	...
	.. exec_code:: realCTsignals signalplots
		...
		G = control.tf([10],[1,10])
		plt.clf()
		w = np.logspace(-1,100,10000)
		ag,phase,omega = control.bode(G,w,Hz=True,dB=True,color='k') 
		
		ax1,ax2 = plt.gcf().axes     # get subplot axes
		plt.sca(ax1)                 # magnitude plot
		plt.plot([0,1],[0,0],'k--')
		plt.plot([1,100],[0,-40],'k--')
		plt.xlim(0.01,100)
		plt.ylim(-21,1)
		
		plt.sca(ax2)                 # magnitude plot
		plt.plot([0,0.1],[0,0],'k--')
		plt.plot([0.1,10],[0,-90],'k--')
		plt.plot([10,100],[-90,-90],'k--')
		plt.xlim(0.01,100)
		plt.ylabel("Fase (deg)")
		
		plt.xlabel("Frequência (rad/s)")
		plt.savefig('source/figures/BodeProva2.png')